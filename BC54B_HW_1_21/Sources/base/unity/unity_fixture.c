...
void (*__FAR outputChar)(char) = TERMIO_PutChar;
...
int UnityMain(int argc,
    ...
    return (int)UnityFailureCount();
}

static int selected(const char *__FAR filter, const char *__FAR name) {
    if (filter == 0) {
        return 1;
    }
...
static int testSelected(const char *__FAR test) {
    return selected(UnityFixture.NameFilter, test);
}

static int groupSelected(const char *__FAR group) {
...
void UnityIgnoreTest(const char *__FAR printableName, const char *__FAR group, const char *__FAR name) {
    if (testSelected(name) && groupSelected(group)) {
        Unity.NumberOfTests++;
...
void *unity_malloc(size_t size) {
    ...
    guard->size = size;
    mem = (char *) & (guard[1]);
    (void)memcpy(&mem[size], end, strlen(end) + 1);
...
void *unity_realloc(void *oldMem, size_t size) {
    ...
    (void)memcpy(newMem, oldMem, guard->size);
    unity_free(oldMem);
    return newMem;
}
...
typedef struct _PointerPair {
    struct _PointerPair *__FAR next;
    void *__FAR *__FAR pointer;
    void *__FAR old_value;
} PointerPair;
...
void UnityPointer_Set(void *__FAR *__FAR pointer, void *__FAR newValue) {
