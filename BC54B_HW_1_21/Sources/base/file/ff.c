...
)
{
    ...
    if (res == FR_OK) {
        ...
        if (disk_ioctl(fs->drv, CTRL_SYNC, 0) != DRES_OK)
            res = FR_DISK_ERR;
    }
    ...
    while (dp->sect) {
        ...
        if (c == DDE || (!_FS_RPATH && c == '.') || (int)(a == AM_VOL) != vol) {    /* An entry without valid data */
            ...
            if (a == AM_LFN) {            /* An LFN entry is found */
                ...
                //dlw 20140707
                if(!( c == '.' || IsUpper(c) || IsLower(c) || IsDigit(c))) res = dir_remove(dp);
                break;
            }
        }
#else        /* Non LFN configuration */
...
)
{
    ...
    *rfs = fs;                            /* Return pointer to the file system object */
    ...
    if (disk_ioctl(fs->drv, GET_SECTOR_SIZE, &fs->ssize) != DRES_OK)
        return FR_DISK_ERR;
#endif
...
)
{
    ...
    if (disk_ioctl(pdrv, GET_SECTOR_SIZE, &SS(fs)) != DRES_OK || SS(fs) > _MAX_SS)
        return FR_DISK_ERR;
#endif
    if (_MULTI_PARTITION && part) {
        ...
        if (disk_ioctl(pdrv, GET_SECTOR_COUNT, &n_vol) != DRES_OK || n_vol < 128)
            return FR_DISK_ERR;
    ...
    if (disk_ioctl(pdrv, GET_BLOCK_SIZE, &n) != DRES_OK || !n || n > 32768) n = 1;
    ...
    return (disk_ioctl(pdrv, CTRL_SYNC, 0) == DRES_OK) ? FR_OK : FR_DISK_ERR;
}
